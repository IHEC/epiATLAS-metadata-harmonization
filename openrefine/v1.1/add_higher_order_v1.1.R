...
#loading IHEC df
main_csv <- 'IHEC_metadata_harmonization.v1.1.extended.csv'
...
for (curie_col in c(ncit_cols, 'harmonized_sample_ontology_curie')){
  ...
  if (curie_col == 'harmonized_sample_ontology_curie') {
    ...
    spread_curies[automated_harmonized_sample_ontology=='EFO', term:=sapply(single_curie, term, object=efo)]
                      ...
                      automated_harmonized_sample_ontology_term := 
                        sapply(strsplit(
                             ...
                             paste(sort(sapply(curies, function(curie)
                               termLabel(ncit_terms[[curie]]))), collapse = separator))]
